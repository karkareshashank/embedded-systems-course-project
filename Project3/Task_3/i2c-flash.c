...
static void my_write_wq_fn(struct work_struct* work)
{
	int i;
	int j;
	int ret;
		...
		size_t count, loff_t *offset)
{
	...
	else if(work->work_status == 0)
	{
		work->buf = memdup_user(buf, count*64);
		work->work_status = 1;
		work->page_num = count;
		work->addr[0] = addr[0];
		work->addr[0] = addr[1];
...
static int i2cdev_attach_adapter(struct device *dev, void *dummy)
{
	...
	pr_debug("i2c_flash-dev: adapter [%s] registered as minor %d\n",
		 adap->name, adap->nr);
	return 0;
error_destroy:
	device_destroy(i2c_dev_class, MKDEV(MY_I2C_MAJOR, adap->nr));
error:
	return_i2c_dev(i2c_dev);
	return res;
}
...
static int i2cdev_detach_adapter(struct device *dev, void *dummy)
{
	...
	pr_debug("i2c_flash-dev: adapter [%s] unregistered\n", adap->name);
	return 0;
}
...
static int __init i2c_dev_init(void)
{
	...
	res = register_chrdev(MY_I2C_MAJOR, "i2c_flash", &i2cdev_fops);
	if (res)
		goto out;

	i2c_dev_class = class_create(THIS_MODULE, "i2c_flash-dev");
